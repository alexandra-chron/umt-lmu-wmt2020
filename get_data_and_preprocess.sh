...
SRC_MODEL_TC=$DATA_PATH/$SRC.tc.model
...
# install tools
#./install-tools.sh (this should have already run in the get_data_mass_pretraining.sh script)
...
if [ "$TGT" == "ro" ]; then
  ...
  TGT_PREPROCESSING="$REPLACE_UNICODE_PUNCT | $NORM_PUNC -l cs | $REM_NON_PRINT_CHAR |                                            $TOKENIZER -l cs -no-escape -threads $N_THREADS"
else
...
# truecasing
if ! [[ -f "$TGT_MODEL_TC" ]]; then
    echo "Training truecase model for $TGT..."
    eval "$TRUE_CASE_TRAIN --model $TGT_MODEL_TC --corpus $TGT_TRAIN_TOK"
fi

if ! [[ -f "$TGT_TRAIN_TC" ]]; then
    echo "Truecase $TGT monolingual data..."
    eval "$TRUE_CASE_APPLY --model $TGT_MODEL_TC < $TGT_TRAIN_TOK > $TGT_TRAIN_TC"
fi

# learn BPE codes on the concatenation of the SRC and TGT datasets
if [ ! -f "$BPE_JOINT_CODES" ]; then
  echo "Learning BPE codes..."
  $FASTBPE learnbpe $CODES $SRC_TRAIN_TC $TGT_TRAIN_TC > $BPE_JOINT_CODES
fi
...
if ! [[ -f "$TGT_TRAIN_BPE" ]]; then
  ...
  $FASTBPE applybpe $TGT_TRAIN_BPE $TGT_TRAIN_TC $BPE_JOINT_CODES
fi
...
if ! [[ -f "$SRC_VALID_TC" ]]; then
    ...
    eval "$TRUE_CASE_APPLY --model $SRC_MODEL_TC < $SRC_VALID_TOK > $SRC_VALID_TC"
fi

if ! [[ -f "$SRC_TEST_TC" ]]; then
    echo "Truecase $SRC monolingual data..."
    eval "$TRUE_CASE_APPLY --model $SRC_MODEL_TC < $SRC_TEST_TOK > $SRC_TEST_TC"
fi

if ! [[ -f "$TGT_VALID_TC" ]]; then
    echo "Truecase $TGT monolingual data..."
    eval "$TRUE_CASE_APPLY --model $TGT_MODEL_TC < $TGT_VALID_TOK > $TGT_VALID_TC"
fi

if ! [[ -f "$TGT_TEST_TC" ]]; then
    echo "Truecase $TGT monolingual data..."
    eval "$TRUE_CASE_APPLY --model $TGT_MODEL_TC < $TGT_TEST_TOK > $TGT_TEST_TC"
fi

echo "Applying BPE to valid and test files..."

$FASTBPE applybpe $SRC_VALID_BPE "$SRC_VALID_TC" $BPE_CODES_HMR
$FASTBPE applybpe $SRC_TEST_BPE  "$SRC_TEST_TC"  $BPE_CODES_HMR

$FASTBPE applybpe $TGT_VALID_BPE "$TGT_VALID_TC" $BPE_JOINT_CODES
$FASTBPE applybpe $TGT_TEST_BPE  "$TGT_TEST_TC"  $BPE_JOINT_CODES
