...
class PredLayer(nn.Module):
    ...
    def forward(self, x, y, get_scores=False):
        ...
        if self.asm is False:
            ...
            # DS: elementwise_mean is deprecated
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, params, dico, is_encoder, with_output):
        ...
        # DS: changed this line so that we always use language embeddings
        ...
        self.use_adapters_enc_dec_attention = params.use_adapters_enc_dec_attention
        if self.use_adapters:
            ...
            for _ in range(self.n_layers):
                ...
                self.adapters_2.append(nn.Sequential(nn.Linear(self.dim, params.adapter_size),
                                                     ...
                                                     nn.Linear(params.adapter_size, self.dim)))
        if self.is_decoder:
            if self.use_adapters_enc_dec_attention:
                self.adapters_3 = nn.ModuleList()
                for _ in range(self.n_layers):
                    self.adapters_3.append(nn.Sequential(nn.Linear(self.dim, params.adapter_size),
                                                         nn.ReLU(),
                                                         nn.Linear(params.adapter_size, self.dim)))
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None, cache=None, enc_mask=None):
        ...
        for i in range(self.n_layers):
            ...
            if self.is_decoder and src_enc is not None:
                ...
                attn = F.dropout(attn, p=self.dropout, training=self.training)
                if self.use_adapters_enc_dec_attention:
                    attn = self.adapters_3[i](attn) + attn
                tensor = tensor + attn
