...
class EncDecTrainer(Trainer):
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff, sample_temperature=None):
        """
        ...
        with torch.no_grad():
            ...
            enc1 = enc1.transpose(0, 1)
            sample_temperature = sample_temperature if sample_temperature is not None \
                                                       and random.random() < self.params.sampling_frequency else None
            x2, len2 = _decoder.generate(enc1, len1, lang2_id, max_len=int(1.3 * len1.max().item() + 5), sample_temperature=sample_temperature)
            langs2 = x2.clone().fill_(lang2_id)
