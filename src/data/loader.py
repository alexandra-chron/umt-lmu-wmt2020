...
def load_mono_data(params, data):
    ...
    for lang in params.mono_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            dico = global_dico if global_dico is not None and lang == params.increase_vocab_for_lang else mono_data[
                'dico']
            set_dico_parameters(params, data, dico) # mono_data['dico'])
...
def load_para_data(params, data):
    ...
    for src, tgt in params.para_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            src_dico = global_dico if global_dico is not None and src == params.increase_vocab_for_lang else src_data[
                'dico']
            tgt_dico = global_dico if global_dico is not None and tgt == params.increase_vocab_for_lang else tgt_data[
                'dico']

            # update dictionary parameters
            set_dico_parameters(params, data, src_dico) # src_data['dico'])
            set_dico_parameters(params, data, tgt_dico) # tgt_data['dico'])
...
def check_data_params(params):
    ...
    logger.info('Modified loading of parallel data. Different datasets are used for different MT directions')
    params.para_dataset = {
        (src, tgt): {
